...
const SimpleRichTextEditor = ({ initialValue, onSave, placeholder, height = "min-h-[150px]" }: { initialValue: string, onSave: (val: string) => void, placeholder?: string, height?: string }) => {
  ...
  return (
    <div className="border border-slate-200 rounded-xl overflow-hidden bg-white shadow-sm flex flex-col w-full">
      ...
      <div ref={contentRef} contentEditable className={`rte-content p-4 flex-1 outline-none text-sm text-slate-700 overflow-y-auto ${height}`} onBlur={handleBlur} dangerouslySetInnerHTML={{ __html: initialValue || '' }} data-placeholder={placeholder} />
    </div>
  );
};
...
const TableRow = ({ cl, idx, cellInputClass, onUpdate, onDelete }: any) => (
  <tr className="border-b border-slate-100 hover:bg-slate-50 transition-colors group">
    ...
    <td className="px-3 py-2"><select value={cl.status} onChange={(e) => onUpdate(cl.id, 'status', e.target.value)} className={`w-full text-[10px] font-bold uppercase rounded py-1 px-1 outline-none cursor-pointer transition-colors bg-transparent hover:bg-slate-100 ${cl.status === 'DONE' ? 'text-emerald-600' : cl.status === 'ON PROGRESS' ? 'text-amber-600' : 'text-slate-400'}`}><option value="NONE">Not Started</option><option value="ON PROGRESS">On Progress</option><option value="DONE">Done</option></select></td>
    <td className="px-3 py-2 text-right"><button onClick={() => onDelete(cl.id)} className="text-slate-300 hover:text-red-500 text-[10px] font-black uppercase transition-colors p-1 opacity-0 group-hover:opacity-100" title="Delete"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button></td>
  </tr>
);
...
const AddRow = ({ newItem, updateNewItem, onAdd, newRowInputClass }: any) => (
  <tr className="bg-slate-50/50 hover:bg-slate-50 transition-colors">
    ...
    <td className="px-3 py-2"><span className="text-[10px] font-bold text-slate-400 italic pl-2">Pending</span></td>
    <td className="px-3 py-2 text-right"><button onClick={onAdd} className="text-indigo-600 bg-indigo-50 px-3 py-1.5 rounded text-[10px] font-black uppercase hover:bg-indigo-100 transition-colors">Add</button></td>
  </tr>
);
...
const ProjectMaster: React.FC<Props> = ({ projects, designers, projectSurveys = [], projectChecklists = [], checklistTemplates = [], checklistTemplateItems = [], onUpdate }) => {
  ...
  const handleCopySurveyLink = () => { const publicUrl = `${window.location.origin}${window.location.pathname}#/portal/v1/survey/${SURVEY_FORM_SECRET}`; navigator.clipboard.writeText(publicUrl); setCopySuccess(true); setTimeout(() => setCopySuccess(false), 2000); };
  ...
  const handleSubmitClarification = async (surveyId: string) => { if (!supabase || !clarificationNote.trim()) return; const { error } = await supabase.from('project_surveys').update({ status: 'CLARIFICATION_REQUESTED', clarification_notes: clarificationNote }).eq('id', surveyId); if (error) alert("Error: " + error.message); else { setIsClarifying(false); setClarificationNote(''); onUpdate(); } };
      ...
      <div className="flex flex-col h-full animate-in slide-in-from-right duration-300 relative">
        {isManageTemplatesOpen && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-sm" onClick={() => setIsManageTemplatesOpen(false)}>
             <div className="bg-white w-full max-w-6xl h-[85vh] rounded-3xl shadow-2xl overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
              <div className="p-6 border-b border-slate-200 bg-slate-50 flex justify-between items-center"><h3 className="text-xl font-black text-slate-900 uppercase tracking-tight">Manage Checklist Templates</h3><button onClick={() => setIsManageTemplatesOpen(false)} className="p-2 bg-white rounded-lg hover:bg-slate-200 text-slate-500"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"/></svg></button></div>
              <div className="flex flex-1 overflow-hidden">
                <div className="w-80 border-r border-slate-200 bg-white p-4 flex flex-col flex-shrink-0">
                   <div className="flex gap-2 mb-4"><input type="text" placeholder="New Template Name" value={newTemplateName} onChange={e => setNewTemplateName(e.target.value)} className="w-full text-xs font-bold p-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-600 outline-none" /><button onClick={handleAddTemplate} className="px-4 bg-indigo-600 text-white rounded-lg text-xs font-bold shadow-sm hover:bg-indigo-700">Add</button></div>
                   <div className="flex-1 overflow-y-auto space-y-2 pr-1">{checklistTemplates.map(t => (<div key={t.id} onClick={() => setSelectedTemplateForEdit(t)} className={`p-3 rounded-lg border cursor-pointer flex justify-between items-center group transition-all ${selectedTemplateForEdit?.id === t.id ? 'bg-indigo-50 border-indigo-300 ring-1 ring-indigo-200' : 'bg-slate-50 border-slate-200 hover:border-indigo-300 hover:bg-white'}`}>{editingTemplateNameId === t.id ? (<div className="flex items-center gap-1 w-full"><input className="w-full text-xs font-bold p-1 border border-indigo-300 rounded bg-white" value={tempTemplateName} onChange={(e) => setTempTemplateName(e.target.value)} onBlur={handleEditTemplateNameSave} onKeyDown={(e) => e.key === 'Enter' && handleEditTemplateNameSave()} autoFocus onClick={(e) => e.stopPropagation()}/></div>) : (<div className="flex items-center gap-2 overflow-hidden flex-1"><span className="text-xs font-black text-slate-800 uppercase truncate">{t.name}</span></div>)}<div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0 ml-2"><button onClick={(e) => { e.stopPropagation(); handleEditTemplateNameStart(t); }} className="p-1 text-slate-400 hover:text-indigo-600 hover:bg-indigo-100 rounded"><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg></button><button onClick={(e) => { e.stopPropagation(); handleDeleteTemplate(t.id); }} className="p-1 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded"><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button></div></div>))}</div></div>
                <div className="flex-1 p-8 bg-slate-50 overflow-y-auto">{selectedTemplateForEdit ? (<div className="space-y-8 max-w-4xl mx-auto"><div className="flex justify-between items-end border-b border-slate-200 pb-4"><div><span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Editing Template</span><h4 className="text-2xl font-black text-slate-900 uppercase tracking-tight">{selectedTemplateForEdit.name}</h4></div><span className="text-xs font-bold text-slate-500 bg-slate-200 px-3 py-1 rounded-full">{checklistTemplateItems.filter(ti => ti.template_id === selectedTemplateForEdit.id).length} Items</span></div><div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm grid grid-cols-4 gap-3"><input type="text" placeholder="Design Name" value={newTemplateItem.task_name} onChange={e => setNewTemplateItem({...newTemplateItem, task_name: e.target.value})} className="col-span-2 text-xs font-bold p-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-indigo-600 outline-none" /><input type="text" placeholder="Size (e.g. A4)" value={newTemplateItem.size} onChange={e => setNewTemplateItem({...newTemplateItem, size: e.target.value})} className="col-span-1 text-xs font-bold p-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-indigo-600 outline-none" /><button onClick={handleAddTemplateItem} className="col-span-1 bg-indigo-600 text-white rounded-xl text-xs font-black uppercase tracking-wide shadow-md hover:bg-indigo-700 transition-all">Add Item</button><input type="text" placeholder="Default Notes (Optional)" value={newTemplateItem.notes} onChange={e => setNewTemplateItem({...newTemplateItem, notes: e.target.value})} className="col-span-4 text-xs font-bold p-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-indigo-600 outline-none" /></div><div className="space-y-3">{checklistTemplateItems.filter(ti => ti.template_id === selectedTemplateForEdit.id).map(ti => (<div key={ti.id} className="bg-white p-4 rounded-xl border border-slate-200 flex justify-between items-start gap-4 hover:shadow-md transition-shadow group"><div className="flex-1 grid grid-cols-12 gap-4"><div className="col-span-5"><span className="text-[9px] font-black text-slate-400 uppercase block mb-1">Item Name</span><input className="w-full text-sm font-black text-slate-900 uppercase bg-transparent border-b border-transparent focus:border-indigo-500 outline-none pb-1 transition-colors" defaultValue={ti.task_name} onBlur={(e) => handleUpdateTemplateItem(ti.id, 'task_name', e.target.value)} placeholder="Item Name"/></div><div className="col-span-3"><span className="text-[9px] font-black text-slate-400 uppercase block mb-1">Size</span><input className="w-full text-xs font-bold text-slate-600 bg-transparent border-b border-transparent focus:border-indigo-500 outline-none pb-1 transition-colors" defaultValue={ti.size} onBlur={(e) => handleUpdateTemplateItem(ti.id, 'size', e.target.value)} placeholder="Size"/></div><div className="col-span-4"><span className="text-[9px] font-black text-slate-400 uppercase block mb-1">Default Notes</span><input className="w-full text-xs font-medium text-slate-500 bg-transparent border-b border-transparent focus:border-indigo-500 outline-none pb-1 transition-colors italic" defaultValue={ti.notes} onBlur={(e) => handleUpdateTemplateItem(ti.id, 'notes', e.target.value)} placeholder="Notes"/></div></div><button onClick={() => handleDeleteTemplateItem(ti.id)} className="text-slate-300 hover:text-red-500 p-2 rounded-lg hover:bg-red-50 transition-all self-center opacity-0 group-hover:opacity-100"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"/></svg></button></div>))}</div></div>) : (<div className="h-full flex flex-col items-center justify-center text-center opacity-60"><div className="w-20 h-20 bg-slate-200 rounded-full flex items-center justify-center mb-4 text-4xl">📝</div><h4 className="text-xl font-black text-slate-400 uppercase">No Template Selected</h4><p className="text-slate-400 text-sm font-medium mt-2">Select a template from the sidebar to manage its items.</p></div>)}</div></div></div>)}
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 pb-4 border-b border-slate-200">
          ...
          <div className="w-full md:w-auto">
             <select value={selectedProject.status} onChange={(e) => handleStatusUpdate(e.target.value)} className={`w-full md:w-48 px-4 py-2.5 rounded-xl border-2 text-sm font-black uppercase outline-none cursor-pointer hover:opacity-90 transition-all shadow-sm appearance-none text-center ${selectedProject.status === 'DONE' ? 'bg-emerald-100 text-emerald-800 border-emerald-300' : selectedProject.status === 'ON HOLD' ? 'bg-amber-100 text-amber-800 border-amber-300' : 'bg-blue-100 text-blue-800 border-blue-300'}`}>
        ...
        <div className="flex gap-1 bg-slate-100 p-1 rounded-xl w-fit mb-6">
          <button onClick={() => setActiveTab('details')} className={`px-6 py-2 rounded-lg text-xs font-black uppercase tracking-wide transition-all ${activeTab === 'details' ? 'bg-white text-indigo-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Project Details</button>
        ...
        <div className="flex-1 overflow-y-auto">
          {activeTab === 'details' && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in duration-300">
               <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm h-fit"><h3 className="text-xs font-black text-slate-900 uppercase tracking-widest mb-4 border-b border-slate-100 pb-2">Information</h3><div className="space-y-4"><div><span className="text-[10px] font-bold text-slate-400 uppercase block mb-1">PIC Designer</span><p className="font-bold text-slate-800 uppercase flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-indigo-600"></span>{getDesignerName(selectedProject.pic_designer_id)}</p></div><div><span className="text-[10px] font-bold text-slate-400 uppercase block mb-1">Support Team</span><div className="flex flex-wrap gap-1">{selectedProject.support_designer_ids && selectedProject.support_designer_ids.length > 0 ? selectedProject.support_designer_ids.map(sid => (<span key={sid} className="px-2 py-0.5 bg-slate-50 text-slate-600 text-[9px] font-black rounded border border-slate-200 uppercase">{getDesignerName(sid)}</span>)) : <span className="text-xs text-slate-400 italic">No support</span>}</div></div><div><span className="text-[10px] font-bold text-slate-400 uppercase block mb-1">Locations</span><div className="flex flex-wrap gap-2">{Array.isArray((selectedProject as any).locations) && (selectedProject as any).locations.length > 0 ? (selectedProject as any).locations.map((loc:string) => <span key={loc} className="px-2 py-1 bg-slate-50 text-[10px] font-black rounded border uppercase">{loc}</span>) : (typeof (selectedProject as any).locations === 'string' && (selectedProject as any).locations ? <span className="px-2 py-1 bg-slate-100 text-[10px] font-black rounded border uppercase">{(selectedProject as any).locations}</span> : <p className="font-bold text-slate-400 text-xs italic">HQ</p>)}</div></div></div></div>
               <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm h-fit lg:col-span-2 min-h-[350px] flex flex-col"><h3 className="text-xs font-black text-slate-900 uppercase tracking-widest mb-4 border-b border-slate-100 pb-2">Notes & Updates</h3><div className="flex-1 flex flex-col"><SimpleRichTextEditor initialValue={selectedProject.notes || ''} onSave={handleNotesUpdate} placeholder="Write project notes here (bold, lists supported)..."/></div></div>
                        ...
                        <div className="flex flex-col gap-6">
                           {isRequested && (
                             <div className="bg-amber-50 border border-amber-200 p-4 rounded-xl flex items-center gap-3">
                               <div className="p-2 bg-amber-100 text-amber-600 rounded-lg"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg></div>
                               <div><p className="text-xs font-black text-amber-800 uppercase">Pending Client Update</p><p className="text-xs text-amber-700 mt-0.5">Waiting for the client to revise their evaluation based on your clarification request.</p></div>
                             </div>
                           )}

                           {survey.evaluator_name && (
                             <div className="bg-indigo-50 border border-indigo-100 p-3 rounded-lg flex justify-between items-center">
                                <span className="text-[10px] font-black text-indigo-400 uppercase tracking-widest">Evaluated By</span>
                                <span className="text-sm font-black text-indigo-900 uppercase">{survey.evaluator_name}</span>
                             </div>
                           )}
                             ...
                             <div className="pt-4 border-t border-slate-100 mt-2">
                                {isClarifying ? (
                                  <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 animate-in fade-in slide-in-from-top-2">
                                    ...
                                    <textarea className="w-full text-xs font-medium p-3 rounded-lg border border-slate-300 focus:ring-2 focus:ring-amber-500 outline-none mb-3" placeholder="Explain what needs to be revised..." rows={3} value={clarificationNote} onChange={(e) => setClarificationNote(e.target.value)}/>
                                    <div className="flex justify-end gap-2"><button onClick={() => setIsClarifying(false)} className="px-3 py-1.5 text-[10px] font-black uppercase text-slate-500 hover:bg-slate-200 rounded">Cancel</button><button onClick={() => handleSubmitClarification(survey.id)} className="px-4 py-1.5 text-[10px] font-black uppercase bg-amber-500 text-white rounded shadow-sm hover:bg-amber-600">Send Request</button></div>
                                  </div>
                                ) : (
          ...
          {activeTab === 'checklist' && (
             <div className="grid grid-cols-1 gap-6 animate-in fade-in duration-300">
                <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center gap-4 flex-wrap"><div className="flex items-center gap-2"><span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Templates:</span><button onClick={() => setIsManageTemplatesOpen(true)} className="text-[10px] font-bold text-indigo-600 underline hover:text-indigo-800">Manage</button></div><div className="h-6 w-px bg-slate-200 mx-2"></div><div className="flex flex-wrap gap-2">{checklistTemplates.map(t => { const isActive = activeTemplatesSet.has(t.id); return ( <button key={t.id} onClick={() => handleToggleTemplate(t.id)} className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase border transition-all ${ isActive ? 'bg-indigo-600 border-indigo-600 text-white shadow-md' : 'bg-slate-50 border-slate-200 text-slate-600 hover:border-indigo-300' }`}> {t.name} {isActive && '✓'} </button> ); })} {checklistTemplates.length === 0 && <span className="text-[10px] text-slate-400 italic">No templates available. Create one in 'Manage'.</span>} </div></div>
                <div className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden"><table className="w-full text-left text-xs border-collapse"><thead className="bg-slate-50 border-b border-slate-200"><tr><th className="px-3 py-2.5 text-center w-10 text-[10px] font-bold text-slate-500 uppercase tracking-wider">#</th><th className="px-3 py-2.5 text-[10px] font-bold text-slate-500 uppercase tracking-wider">Design Spec</th><th className="px-3 py-2.5 w-32 text-[10px] font-bold text-slate-500 uppercase tracking-wider">Size</th><th className="px-3 py-2.5 text-center w-16 text-[10px] font-bold text-slate-500 uppercase tracking-wider">Qty</th><th className="px-3 py-2.5 text-[10px] font-bold text-slate-500 uppercase tracking-wider">Notes</th><th className="px-3 py-2.5 w-32 text-[10px] font-bold text-slate-500 uppercase tracking-wider">Status</th><th className="px-3 py-2.5 text-right w-16 text-[10px] font-bold text-slate-500 uppercase tracking-wider"></th></tr></thead><tbody className="text-slate-700">{sortedActiveTemplateIds.map(templateId => { const items = groupedChecklists.groups[templateId] || []; const templateName = checklistTemplates.find(t => t.id === templateId)?.name || 'Unknown Template'; const newItemState = newItemsMap[templateId] || { task_name: '', size: '', quantity: 1, notes: '' }; return ( <React.Fragment key={templateId}> <tr className="bg-indigo-50 border-y border-indigo-100"> <td colSpan={7} className="px-3 py-1.5"> <span className="text-[10px] font-black text-indigo-800 uppercase tracking-widest flex items-center gap-2"> <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/></svg> {templateName} </span> </td> </tr> {items.map((cl, idx) => ( <TableRow key={cl.id} cl={cl} idx={idx} cellInputClass={cellInputClass} onUpdate={handleUpdateChecklistField} onDelete={handleDeleteChecklist} /> ))} <AddRow newItem={newItemState} updateNewItem={(field: any, val: any) => updateNewItemState(templateId, field, val)} onAdd={() => handleAddNewItem(templateId)} newRowInputClass={newRowInputClass} /> </React.Fragment> ); })} <React.Fragment key="manual"> <tr className="bg-slate-100 border-y border-slate-200"> <td colSpan={7} className="px-3 py-1.5"> <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2"> <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"/></svg> Additional / Manual Items </span> </td> </tr> {groupedChecklists.manualItems.map((cl, idx) => ( <TableRow key={cl.id} cl={cl} idx={idx} cellInputClass={cellInputClass} onUpdate={handleUpdateChecklistField} onDelete={handleDeleteChecklist} /> ))} <AddRow newItem={newItemsMap['manual'] || { task_name: '', size: '', quantity: 1, notes: '' }} updateNewItem={(field: any, val: any) => updateNewItemState(null, field, val)} onAdd={() => handleAddNewItem(null)} newRowInputClass={newRowInputClass} /> </React.Fragment> </tbody></table></div>
             </div>
          )}
        </div>
      </div>
  ...
  return (
    <div className="space-y-6 flex flex-col h-full relative">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 flex-shrink-0"><div><h1 className="text-2xl font-black text-slate-900 tracking-tight uppercase">Project Master</h1><p className="text-slate-600 text-sm mt-1 font-bold">Manage event project timelines.</p></div><div className="flex items-center gap-4"><button onClick={handleCopySurveyLink} className={`px-4 py-2 rounded-lg text-xs font-bold uppercase transition-all flex items-center gap-2 border ${copySuccess ? 'bg-emerald-50 border-emerald-500 text-emerald-700' : 'bg-white border-slate-300 text-slate-700 hover:border-indigo-500'}`}><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"/></svg>{copySuccess ? 'Survey Link Copied!' : 'Eval Survey Link'}</button><div className="flex bg-slate-200 p-1 rounded-xl"><button onClick={() => setView('list')} className={`px-4 py-1.5 rounded-lg text-xs font-black transition-all ${view === 'list' ? 'bg-white text-indigo-700 shadow-sm' : 'text-slate-600'}`}>List</button><button onClick={() => setView('calendar')} className={`px-4 py-1.5 rounded-lg text-xs font-black transition-all ${view === 'calendar' ? 'bg-white text-indigo-700 shadow-sm' : 'text-slate-600'}`}>Calendar</button></div>{!isAdding && view === 'list' && (<button onClick={() => setIsAdding(true)} className="px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-black shadow-lg">Add Project</button>)}</div></header>
      <div className="bg-slate-100 p-4 rounded-2xl flex flex-wrap items-center gap-4 border border-slate-200">{[['Status', filterStatus, setFilterStatus, ['ALL', 'ON PROGRESS', 'ON HOLD', 'DONE']], ['Type', filterType, setFilterType, ['ALL', 'EVENT', 'TRAVEL', 'WELLNESS', 'CREATIVE', 'TRAINING']], ['PIC', filterPIC, setFilterPIC, ['ALL', ...designers.map(d=>d.id)]], ['Location', filterLocation, setFilterLocation, ['ALL', ...uniqueLocations]]].map(([lbl, val, set, opts]: any) => (<div key={lbl as string} className="flex flex-col gap-1"><span className="text-[9px] font-black text-slate-400 uppercase tracking-widest px-1">{lbl as string}</span><select value={val as string} onChange={e => (set as any)(e.target.value)} className="text-[10px] font-bold border-slate-200 rounded-lg p-2 bg-white outline-none focus:ring-2 focus:ring-indigo-500 uppercase tracking-tighter cursor-pointer">{opts.map((o:any) => <option key={o} value={o}>{o === 'ALL' ? `All ${lbl}` : (lbl === 'PIC' ? getDesignerName(o) : o)}</option>)}</select></div>))}</div>
      {isAdding && (<form onSubmit={handleSave} className="bg-white p-8 rounded-2xl border border-slate-200 shadow-xl animate-in zoom-in duration-200 flex-shrink-0 mb-6"><h2 className="font-black text-slate-900 mb-8 uppercase tracking-tight flex items-center gap-2"><span className="w-2 h-2 bg-indigo-600 rounded-full"></span>{editingId ? 'Edit Project' : 'New Project'}</h2><div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8"><div className="md:col-span-2"><label className={labelClass}>Project Name</label><input type="text" required value={formData.project_name} onChange={e => setFormData({...formData, project_name: e.target.value})} className={inputClass} placeholder="Annual Event 2024" /></div><div><label className={labelClass}>Status</label><select value={formData.status} onChange={e => setFormData({...formData, status: e.target.value as any})} className={inputClass}><option value="ON PROGRESS">ON PROGRESS</option><option value="ON HOLD">ON HOLD</option><option value="DONE">DONE</option></select></div><div><label className={labelClass}>Project Type</label><select value={formData.project_type} onChange={e => setFormData({...formData, project_type: e.target.value})} className={inputClass}><option value="EVENT">EVENT</option><option value="TRAVEL">TRAVEL</option><option value="WELLNESS">WELLNESS</option><option value="CREATIVE">CREATIVE</option><option value="TRAINING">TRAINING</option></select></div><div><label className={labelClass}>Start Date</label><input type="date" required value={formData.start_date} onChange={e => setFormData({...formData, start_date: e.target.value})} className={inputClass} /></div><div><label className={labelClass}>End Date</label><input type="date" required value={formData.end_date} onChange={e => setFormData({...formData, end_date: e.target.value})} className={inputClass} /></div><div className="md:col-span-1"><label className={labelClass}>Locations</label><div className="flex gap-2 mb-2"><input type="text" list="loc-suggestions" placeholder="Pilih/Ketik..." value={newLocInput} onChange={e => setNewLocInput(e.target.value)} className={inputClass} onKeyDown={e => e.key === 'Enter' && (e.preventDefault(), addLocation())} /><button type="button" onClick={addLocation} className="px-5 bg-slate-900 text-white rounded-lg text-sm font-black uppercase tracking-widest">ADD</button></div><div className="flex flex-wrap gap-2 p-3 bg-slate-50 border border-slate-200 rounded-xl min-h-[58px] items-center">{formData.locations?.map(loc => (<span key={loc} className="flex items-center gap-2 px-3 py-1.5 bg-white border border-slate-300 text-slate-800 rounded-lg text-[10px] font-black uppercase shadow-sm">{loc}<button type="button" onClick={() => removeLocation(loc)} className="text-red-500"><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M6 18L18 6M6 6l12 12"/></svg></button></span>))}</div><datalist id="loc-suggestions">{uniqueLocations.map(loc => <option key={loc} value={loc} />)}</datalist></div><div><label className={labelClass}>PIC Designer</label><select value={formData.pic_designer_id} onChange={e => setFormData({...formData, pic_designer_id: e.target.value})} className={inputClass}>{designers.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}</select></div><div className="md:col-span-1"><label className={labelClass}>Support Designers</label><div className="flex flex-wrap gap-2 p-3 bg-slate-50 border border-slate-200 rounded-xl min-h-[58px]">{designers.map(d => { if (d.id === formData.pic_designer_id) return null; const isSelected = formData.support_designer_ids?.includes(d.id); return (<button key={d.id} type="button" onClick={() => toggleSupportDesigner(d.id)} className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase border transition-all ${isSelected ? 'bg-indigo-600 border-indigo-600 text-white shadow-md' : 'bg-white border-slate-300 text-slate-500 hover:border-indigo-400'}`}>{d.name}</button>); })}</div></div><div className="md:col-span-3"><label className={labelClass}>Notes / Keterangan</label><SimpleRichTextEditor initialValue={formData.notes || ''} onSave={(val) => setFormData({...formData, notes: val})} placeholder="Catatan project (bisa format list, bold, dll)..."/></div></div><div className="flex justify-end gap-4 pt-6 border-t border-slate-100"><button type="button" onClick={resetForm} className="px-6 py-2.5 text-sm font-black text-slate-700 uppercase">Cancel</button><button type="submit" className="px-8 py-2.5 bg-indigo-600 text-white rounded-lg text-sm font-black shadow-lg uppercase tracking-widest">{editingId ? 'Update' : 'Save'}</button></div></form>)}
      <div className="flex-1 min-h-0">{view === 'list' ? (<div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col h-full"><div className="overflow-y-auto max-h-full"><table className="w-full text-left text-sm border-collapse"><thead className="sticky top-0 z-10 bg-slate-100 font-black uppercase text-[10px] tracking-wider border-b border-slate-200"><tr><th className="px-6 py-4">Status & Name</th><th className="px-6 py-4">Timeline & Loc</th><th className="px-6 py-4">Lead & Team</th><th className="px-6 py-4 text-right">Actions</th></tr></thead><tbody className="divide-y divide-slate-200 font-bold text-slate-900">{filteredProjects.map(p => { const locs = (p as any).locations || (p as any).location || []; const normalizedLocs = Array.isArray(locs) ? locs : [locs]; return (<tr key={p.id} onClick={() => { setSelectedProject(p); setActiveTab('details'); }} className="hover:bg-slate-50 transition-colors cursor-pointer"><td className="px-6 py-4"><div className="flex flex-col gap-2"><span className={`px-2 py-0.5 rounded-full border text-[8px] font-black uppercase self-start ${getStatusBadge(p.status)}`}>{p.status}</span><span className="font-black uppercase">{p.project_name}</span></div></td><td className="px-6 py-4"><div className="flex flex-col"><span className="text-[11px] font-black">{p.start_date} → {p.end_date}</span><div className="flex flex-wrap gap-1 mt-1">{normalizedLocs.map(l => <span key={l} className="text-[8px] bg-slate-100 px-1.5 py-0.5 rounded border uppercase">{l}</span>)}</div></div></td><td className="px-6 py-4"><div className="flex flex-col gap-1.5"><div className="flex items-center gap-1.5"><span className="w-2 h-2 bg-indigo-600 rounded-full"></span><span className="text-xs uppercase">{getDesignerName(p.pic_designer_id)}</span></div><div className="flex flex-wrap gap-1">{p.support_designer_ids?.map(sid => (<span key={sid} className="px-1.5 py-0.5 bg-slate-50 border border-slate-200 text-[8px] rounded uppercase font-bold text-slate-400">{getDesignerName(sid)}</span>))}</div></div></td><td className="px-6 py-4 text-right"><div className="flex justify-end gap-4"><button onClick={(e) => { e.stopPropagation(); handleEdit(p); }} className="text-indigo-700 text-[10px] font-black uppercase">Edit</button><button onClick={(e) => { e.stopPropagation(); handleDelete(e, p.id); }} className="text-red-500 text-[10px] font-black uppercase">Delete</button></div></td></tr>); })}</tbody></table></div></div>) : (<div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden h-full flex flex-col"><div className="p-4 border-b border-slate-200 bg-slate-50 flex items-center justify-between"><h3 className="font-black text-slate-900 text-sm uppercase">{monthNames[currentDate.getMonth()]} {currentDate.getFullYear()}</h3><div className="flex gap-2"><button onClick={() => setCurrentDate(new Date(currentDate.getFullYear(), currentDate.getMonth()-1, 1))} className="p-1.5 hover:bg-slate-300 rounded-lg"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M15 19l-7-7 7-7"/></svg></button><button onClick={() => setCurrentDate(new Date(currentDate.getFullYear(), currentDate.getMonth()+1, 1))} className="p-1.5 hover:bg-slate-300 rounded-lg"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M9 5l7 7-7 7"/></svg></button></div></div><div className="overflow-y-auto flex-1"><div className="grid grid-cols-7 border-l border-slate-200">{renderCalendar()}</div></div></div>)}</div>
    </div>
  );
};
