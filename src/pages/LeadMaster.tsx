      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 flex-shrink-0">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex bg-[#FAFAFA]200 p-1 rounded-lg">
            ...
            <button onClick={() => setView('board')} className={`px-4 py-1 rounded-md text-xs font-bold ${view === 'board' ? 'bg-white shadow-sm text-zinc-800' : 'text-zinc-600'}`}>Board</button>
              ...
              {Object.keys(leadBoardGroups).sort().map(groupKey => (
                <div key={groupKey} className="w-80 flex-shrink-0 bg-zinc-50/50 rounded-2xl flex flex-col max-h-full border border-zinc-100 shadow-sm">
                  <div className="p-4 border-b border-zinc-100 uppercase tracking-tight font-bold text-sm text-zinc-800 flex justify-between items-center bg-white rounded-t-2xl">
                    ...
                    <span className="bg-zinc-100 text-zinc-500 text-[10px] px-2 py-0.5 rounded-full">{leadBoardGroups[groupKey].length}</span>
                  </div>
