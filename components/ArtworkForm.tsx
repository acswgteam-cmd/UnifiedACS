...
const ArtworkForm: React.FC<Props> = ({ state, onSubmit }) => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    setFormData(prev => ({ 
      ...prev, 
      [name]: val,
      ...(name === 'work_context' ? { project_id: '', lead_id: '', department_id: '' } : {})
    }));
  };

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const newLog: ArtworkLog = {
      ...formData as ArtworkLog,
      id: `log-${Date.now()}`
    };
    onSubmit(newLog);
    setFormData(prev => ({
        ...
        {formData.work_context === WorkContext.PROJECT && (
          <div className="flex items-center space-x-3">
            <input 
              ...
              className="w-5 h-5 text-indigo-600 border-slate-300 rounded focus:ring-indigo-600 bg-white" 
            />
            <label htmlFor="approval_required" className="text-sm font-black text-slate-900 cursor-pointer">Require Approval</label>
          </div>
        )}
