...
const SimpleRichTextEditor = ({ initialValue, onSave, placeholder, height = "min-h-[150px]" }: { initialValue: string, onSave: (val: string) => void, placeholder?: string, height?: string }) => {
  ...
  const btnClass = "p-1.5 rounded hover:bg-[#FAFAFA]200 text-zinc-600 transition-colors text-xs font-bold min-w-[24px]";
  return (
    <div className="border border-[#EAEAEA] rounded-xl overflow-hidden bg-white shadow-sm flex flex-col w-full">
      ...
      <div className="flex gap-1 p-2 bg-[#FCFCFC] border-b border-zinc-100 items-center shrink-0">
        ...
        <div className="w-px h-4 bg-[#FAFAFA]300 mx-1"></div>
      ...
      <div ref={contentRef} contentEditable className={`rte-content p-4 flex-1 outline-none text-sm text-zinc-700 overflow-y-auto ${height}`} onBlur={handleBlur} dangerouslySetInnerHTML={{ __html: initialValue || '' }} data-placeholder={placeholder} />
    </div>
  );
};
...
IMPORTANT: Extract ALL rows. Return raw JSON only, no explanations.`;
  ...
  const inputClass = "w-full rounded-lg border-zinc-300 text-zinc-900 text-sm p-3 border bg-white focus:ring-2 focus:ring-indigo-600 outline-none placeholder-slate-400 font-semibold shadow-sm transition-all";
  ...
  const newRowInputClass = "w-full bg-white border border-[#EAEAEA] rounded-sm focus:border-indigo-600 outline-none text-xs text-zinc-700 py-1.5 px-2";
