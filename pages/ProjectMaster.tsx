...
// Month names for display in calendar view
...
const ProjectMaster: React.FC<Props> = ({ projects, designers, onUpdate }) => {
  ...
  const uniqueLocations = useMemo(() => {
    ...
    projects.forEach(p => {
      // Cek kedua kemungkinan properti (plural/singular) dari DB
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    // Pastikan input lokasi terakhir ditambahkan jika user lupa klik ADD
    ...
    const savePayload = {
      ...
      locations: finalLocations, // Mengirim ke kolom baru 'locations'
    ...
    if (editingId) {
      ...
      if (error) {
        if (error.message.includes("schema cache")) {
          alert("EROR DATABASE: Kolom 'locations' (jamak) belum ditemukan di database Anda. Silakan jalankan SQL migration di dashboard Supabase terlebih dahulu.");
        } else {
  ...
  const handleEdit = (p: Project) => {
    // Normalisasi: deteksi data dari properti tunggal (lama) atau jamak (baru)
    let rawLocs = (p as any).locations || (p as any).location || [];
    let normalizedLocations: string[] = [];
    
    if (Array.isArray(rawLocs)) {
      normalizedLocations = rawLocs;
    } else if (typeof rawLocs === 'string' && rawLocs.trim() !== '') {
      normalizedLocations = [rawLocs.trim()];
    }
  ...
  // Fixed: Added implementation for handleDelete
  ...
  const handleWheel = (e: React.WheelEvent) => {
    ...
    if (Math.abs(e.deltaY) > 20) {
      setCurrentDate(prev => new Date(prev.getFullYear(), prev.getMonth() + (e.deltaY > 0 ? 1 : -1), 1));
        ...
        <div key={d} className={`min-h-[160px] h-full border-r border-b border-slate-200 p-0 flex flex-col relative ${isToday ? 'bg-indigo-50/30' : 'bg-white'}`}>
          ...
          <div className="flex flex-col space-y-1 pb-2 flex-1">
            {calendarLanes.map((lane, laneIdx) => {
              ...
              const displayLocs = Array.isArray((project as any).locations) ? (project as any).locations.join(', ') : ((project as any).location || 'HQ');

              return (
                <div key={project.id} onClick={() => setSelectedProject(project)} className={`cursor-pointer min-h-[58px] py-1.5 flex flex-col justify-center px-2 overflow-hidden transition-all hover:brightness-95 ${theme.bg} ${theme.text} ${isStart ? `rounded-l-md ml-1 border-l-4 ${theme.border}` : ''} ${isEnd ? 'rounded-r-md mr-1' : ''}`}>
                  ...
                  <span className="text-[7px] font-bold opacity-70 mt-0.5 truncate uppercase italic">Loc: {displayLocs}</span>
                </div>
              );
            })}
          </div>
  ...
  return (
    <div className="space-y-6 flex flex-col h-full relative">
      ...
      <div className="bg-slate-100 p-4 rounded-2xl flex flex-wrap items-center gap-4 border border-slate-200">
        {/* Fixed: Cast destructuring arguments to any to avoid property 'map' error on the union type of filter options */}
        ...
        <form onSubmit={handleSave} className="bg-white p-8 rounded-2xl border border-slate-200 shadow-xl animate-in zoom-in duration-200 flex-shrink-0 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
            ...
            <div className="md:col-span-2">
              <label className={labelClass}>Locations (Dropdown dari data lama + Input Manual)</label>
              <div className="flex gap-2 mb-2">
                <input type="text" list="loc-suggestions" placeholder="Pilih dari dropdown atau ketik manual..." value={newLocInput} onChange={e => setNewLocInput(e.target.value)} className={inputClass} onKeyDown={e => e.key === 'Enter' && (e.preventDefault(), addLocation())} />
              ...
              <div className="flex flex-wrap gap-2 p-3 bg-slate-50 border border-slate-200 rounded-xl min-h-[58px] items-center">
                {(!formData.locations || formData.locations.length === 0) && <span className="text-[10px] text-slate-400 font-bold italic uppercase px-1">Belum ada lokasi ditambahkan.</span>}
                {formData.locations?.map(loc => (
                  <span key={loc} className="flex items-center gap-2 px-3 py-1.5 bg-white border border-slate-300 text-slate-800 rounded-lg text-[10px] font-black uppercase shadow-sm">
                    {loc}
                    <button type="button" onClick={() => removeLocation(loc)} className="text-red-500 hover:text-red-700"><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M6 18L18 6M6 6l12 12"/></svg></button>
                  </span>
                ))}
              </div>
            ...
            <div className="md:col-span-3"><label className={labelClass}>Notes</label><textarea rows={3} value={formData.notes || ''} onChange={e => setFormData({...formData, notes: e.target.value})} className={inputClass} placeholder="Catatan project..." /></div>
          </div>
            ...
            <div className="overflow-y-auto max-h-full">
              <table className="w-full text-left text-sm border-collapse">
                <thead className="sticky top-0 z-10 bg-slate-100 font-black uppercase text-[10px] tracking-wider border-b border-slate-200">
                  <tr><th className="px-6 py-4">Status & Name</th><th className="px-6 py-4">Timeline & Loc</th><th className="px-6 py-4">Lead</th><th className="px-6 py-4 text-right">Actions</th></tr>
                </thead>
                  ...
                  {filteredProjects.map(p => {
                    ...
                    return (
                      <tr key={p.id} onClick={() => setSelectedProject(p)} className="hover:bg-slate-50 transition-colors cursor-pointer">
                        ...
                        <td className="px-6 py-4 flex items-center gap-1.5"><span className="w-2 h-2 bg-indigo-600 rounded-full"></span><span className="text-xs uppercase">{getDesignerName(p.pic_designer_id)}</span></td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 backdrop-blur-sm bg-slate-900/40" onClick={() => setSelectedProject(null)}>
          <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl p-8 animate-in zoom-in duration-200" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4"><div><span className="text-[10px] font-bold text-slate-400 uppercase">PIC</span><p className="font-bold text-slate-800">{getDesignerName(selectedProject.pic_designer_id)}</p></div><div><span className="text-[10px] font-bold text-slate-400 uppercase">Timeline</span><p className="font-bold text-slate-800 text-xs uppercase">{selectedProject.start_date} to {selectedProject.end_date}</p></div></div>
              ...
              <div><span className="text-[10px] font-bold text-slate-400 uppercase">Notes</span><div className="p-4 bg-slate-50 rounded-xl text-sm italic text-slate-700 whitespace-pre-wrap">{selectedProject.notes || 'No notes.'}</div></div>
            </div>
