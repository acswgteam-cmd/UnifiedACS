...
const DesignerMaster: React.FC<Props> = ({ designers, onUpdate }) => {
  ...
  const inputClass = "w-full p-2 text-xs font-bold border border-zinc-300 rounded mb-2 focus:ring-2 focus:ring-[var(--primary)] outline-none";
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button onClick={() => setIsAdding(true)} className="px-4 py-2 bg-[var(--primary)] text-white rounded-lg text-xs font-bold uppercase shadow-sm border border-transparent hover:brightness-110 transition-all flex items-center gap-2">
      ...
      {isAdding && (
        <form onSubmit={handleAdd} className="bg-white p-3 md:p-4 rounded-xl border border-[#EAEAEA] shadow-sm animate-in slide-in-from-top duration-200 max-w-2xl">
          ...
          <div className="flex justify-end gap-2">
            ...
            <button type="submit" className="px-4 py-2 bg-[var(--primary)] text-white rounded-lg font-bold text-xs uppercase shadow hover:brightness-110">Save</button>
          </div>
        </form>
      )}
            ...
            {editingId === d.id ? (
              <div className="space-y-2">
                ...
                <div className="flex gap-1 pt-1">
                  <button onClick={saveEdit} className="flex-1 bg-[var(--primary)] text-white text-[9px] font-bold py-1 rounded">Save</button>
